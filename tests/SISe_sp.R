...
res <- tools::assertError(SISe_sp(init     = init,
                                  tspan    = seq_len(10) - 1,
                                  events   = NULL,
                                  phi      = rep(1, nrow(init)),
                                  gamma    = 0.1,
...
res <- tools::assertError(.Call(SimInf:::SISe_sp_run, NULL, NULL, NULL))
stopifnot(length(grep("Invalid SISe_sp model",
                      res[[1]]$message)) > 0)
res <- tools::assertError(.Call(SimInf:::SISe_sp_run, "SISe_sp", NULL, NULL))
stopifnot(length(grep("Invalid SISe_sp model",
                      res[[1]]$message)) > 0)
