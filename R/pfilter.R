...
setMethod(
    ...
    function(model, obs_process, init_model = NULL) {
        if (any(
            isFALSE(identical(dim(model@model@U_sparse), c(0L, 0L))),
            isFALSE(identical(dim(model@model@V_sparse), c(0L, 0L))),
            isFALSE(identical(dim(model@multi_model@U_sparse), c(0L, 0L))),
            isFALSE(identical(dim(model@multi_model@V_sparse), c(0L, 0L))))) {
            stop("'pfilter' cannot run a model with a sparse result matrix.",
                 call. = FALSE)
        }

        npart <- check_npart(model@n_models)
        obs_process <- match.fun(obs_process)
        if (!is.null(init_model))
            init_model <- match.fun(init_model)

        Nc <- Nc(model@multi_model)
        Nd <- nrow(model@multi_model@v0)
        Ntspan <- nrow(model@tspan)
        n_nodes <- n_nodes(model@multi_model)
        ess <- numeric(Ntspan)
        loglik <- 0
        w <- numeric(npart)

        ## Create a matrix to keep track of the states in the
        ## compartments (including the initial state) in every
        ## particle.
        U <- matrix(data = NA_integer_,
                    nrow = prod(dim(model@multi_model@u0)),
                    ncol = Ntspan + 1L)

        ## Create a matrix to keep track of the continuous states
        ## (including the initial state) in every particle.
        V <- matrix(data = NA_real_,
                    nrow = prod(dim(model@multi_model@v0)),
                    ncol = Ntspan + 1L)
        ...
        if (is.function(init_model)) {
            ...
            for (p in seq_len(npart)) {
                ## Initialise the model.
                m <- init_model(model@model)

                ## Save the initial u0 state.
                u_i <- seq.int(from = (p - 1L) * prod(dim(model@model@u0)) + 1L,
                               length.out = prod(dim(model@model@u0)))
                U[u_i, 1L] <- as.integer(m@u0)

                ## Save the initial v0 state.
                v_i <- seq.int(from = (p - 1L) * prod(dim(model@model@v0)) + 1L,
                               length.out = prod(dim(model@model@v0)))
                V[v_i, 1L] <- as.numeric(m@v0)
            }
        } else {
            U[, 1L] <- model@multi_model@u0
            V[, 1L] <- model@multi_model@v0
        }
        ...
        m <- model@multi_model
        for (i in seq_len(Ntspan)) {
            if (is.na(model@tspan[i, 1L])) {
                m@tspan <- model@tspan[i, 2L]
            } else {
                m@tspan <- model@tspan[i, 1:2]
            }

            ## Initialise events for the interval.
            if (!is.null(model@events))
                m@events <- model@events[[i]]

            ## Initialise the model.
            m@u0 <- matrix(data = U[, i],
                           nrow = nrow(m@u0),
                           ncol = ncol(m@u0),
                           dimnames = dimnames(m@u0))

            m@v0 <- matrix(data = V[, i],
                           nrow = nrow(m@v0),
                           ncol = ncol(m@v0),
                           dimnames = dimnames(m@v0))
            ...
            ## Set the weight for the particles.
            w <- obs_process(x, model@data[[i]])
            if (!all(is.finite(w)) && length(w) == npart)
                stop("Invalid observation process.", call. = FALSE)
        ...
        ## Sample a trajectory.
        pfilter_sample_trajectory(model@model, U, V, a, npart, loglik, ess)
   }
)
