...
stochastic process in one node does not affect any other node in the
model.  It is therefore straightforward to run many realizations of
this model, simply by replicating a node in \code{u0}, for example, $n
= 1000$ times.
...
R = rep(0, n))
model <- init(model = m, u0 = u0, tspan = 1:150)
result <- run(model = model, threads = 1, seed = 22)
@

Let us compare the incidence cases in the first node with the average
incidence cases among all realizations.  This is done by extracting
the trajectory and calculate successive differences at each time-point
of \code{'Icum'}.
...
twenty-one after introduction and that we wish to explore how
vaccination could limit the outbreak, if resources for vaccination can
handle 50 herds per day and 80\% of the animals in each herd.  Let us
add one compartment $V$ to the model, to represent vaccinated
individuals, so that the model now contains the $\{S, I, I_{cum}, R, V
\}$ compartments.  As before, let \code{b} denote the transmission
rate and \code{g} the recovery rate.
...
continue until all herds are vaccinated on day fifty-two, and use
