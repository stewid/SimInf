...
run the simulations efficiently.  Let us explore the dynamics of the
...
the movements.  On day 25, 10 individuals are moved to the node
...
because of the random selection process that select susceptible,
infected or recovered individuals to move between the nodes.
...
model, which is also built into the \pkg{SimInf} package.  This model
...
living pathogens (Figure~\ref{fig:SISe3_sp}).  Moreover, nodes are
spatially connected with local spread of the contaminated
environmental compartment among proximal nodes.  The transmission
route of infection to susceptible individuals is indirect via the
local environment, contaminated by infected individuals.  The number
of individuals in each of the six compartments $\{S_1, I_1, S_2, I_2,
S_3, I_3\}$ in node $i$ at time $t$ is \mbox{\code{u[, i]}}.  The
\code{SISe3\_sp} model has two state transitions within each of the
...
The current design supports two ways to extend it, and this section
desribes the relevant steps to implement a new model.  Since both ways
require that \proglang{C} code can be compiled, you will first need to
install a compiler.
...
generates \proglang{C} code for the model and the two matrices
\code{S} and \code{G}.  The left hand side of the first
'\code{->}'-sign is the initial state, the right hand side of the
second '\code{->}'-sign is the final state, and the propensity is
written between the '\code{->}'-signs.  The special symbol '\code{@}'
is reserved for the empty set $\emptyset$.  We suggest to first draw a
...
We can now use the transitions and compartments, together with the
constants \code{b} and \code{g} to build an object of class
\code{'SimInf\_mparse'} via a call to \code{mparse}.  Before we can
simulate from the model, it needs to be initialised with the initial
condition \code{u0} and \code{tspan}.
...
The \code{model} object can now be used as in earlier examples to
simulate data from the model and then plot the results.
