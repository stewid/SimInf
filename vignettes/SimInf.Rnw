...
\Abstract{
  ...
  Livestock are partioned into spatially segregated subpopulations
  (farms).  Thus demography within each subpopulation and interactions
  between subpopulations, for example, by livestock movements, can
  affect transmission dynamics of a contagion.  The \proglang{R}
  package \pkg{SimInf} provides an efficient and very flexible
  framework to exploit rich livestock data in realistic large scale
  disease spread simulations.  The framework integrates infection
  dynamics in each subpopulation as continuous-time Markov chains
  using the Gillespie stochastic simulation algorithm and incorporates
  available data such as births, slaughter and livestock movements as
  scheduled events.  A scheduled event is used to modify the state of
  a subpopulation at a predefined time-point.  Using \proglang{C} code
  for the numerical solvers and \proglang{OpenMP} to divide work over
  multiple processors ensures high performance when simulating a
  trajectory.  One of our design goal was to make \pkg{SimInf}
  extendable and enable usage of the numerical solvers from other
  \proglang{R} extension packages in order to facilitate complex
  epidemiological research.  In this paper, we provide a technical
  description of the framework and demonstrate some basic toy
  examples.  We also discuss how to specify and extend the framework
...
\citep{Pineda-Krch2008}, on CRAN since 2007, but appears not actively
maintained after 2012, implements both the direct method and three
...
\code{data.frame} with the initial number individuals in each
...
\code{show} method of the \code{SimInf_model} class prints some basic
information about the model, such as the global data parameters and
the extremes, the mean and the quartiles of the count in each
...
The \code{plot} method of the \code{SimInf_model} class can be used to
visualize the simulated trajectory.  The default plot will display the
median count in each compartment across nodes as a colored line
together with the inter-quartile range using the same color, but with
transparency.  To display the outcome for individual nodes, use the
\code{node} parameter to specify the subset of nodes to plot and set
...
\begin{figure}
  ...
  \caption{Output from a stochastic \code{SIR} model in 1000 nodes
    ...
    Left: The default plot shows the median and inter-quartile range
    of the count in each compartment through time across all nodes.
    Right: Realizations from a subset of 10
    nodes.  \label{fig:SIR-model}}
\end{figure}
...
incorporated into the simulation.  In order to process a scheduled
event, the compartments that are involved must be defined in the
select matrix \code{E}.  Moreover, all events must specify the column
to use in \code{E}, where the non-zero entries in the specified column
define involved compartments.  For the predefined \code{SIR} model,
\code{E} is defined as
...
three compartments.  Consider we have 4 scheduled events to include in
a simulation.  Below is a \code{data.frame}, that contains the events.

<<SIR-events, echo=FALSE>>=
events <- data.frame(
event = c("enter", "extTrans", "exit", "enter"),
time = c(2, 3, 4, 4), node = c(3, 1, 2, 1),
dest = c(0, 3, 0, 3), n = c(5, 7, 3, 1),
proportion = c(0, 0, 0, 0), select = c(1, 4, 4, 2),
shift = c(0, 0, 0, 0))
@
...
\item In time step 3 we sample 7 individuals among the S, I and R
  compartments in node 1 and move them to the corresponding
  compartments in node 3
\item In time step 4 we sample 3 individuals among the S, I and R
  compartments in node 2 and remove them from node 2.
\item In time step 4 we add 1 infected individual to node 2
\end{enumerate}
