...
algorithm processes these two classes of events differently.  The
scheduled events are processed when simulation time reaches the time
for any of the events.  Events that are scheduled at the same time are
processed in the following order: \textit{exit}, \textit{enter},
\textit{internal transfer} and \textit{external transfer}.
