...
synthetic dataset (\code{nodes}) included in the \pkg{SimInf} package,
wich define a cattle population consisting of 1600 herds located in a
...
trajectory have a seasonal pattern around 10\%.  We assume that the
average duration of infection is 10 days in all age categories, such
that the recovery rate $\gamma_{1,2,3} = 0.1$ per animal per day, and
assume the indirect transmission rate of the environmental infectious
...
\texttt{beta\_t1}, ..., \texttt{beta\_t4} is determined by
\texttt{end\_t1}, ..., \texttt{end\_t4}, respectively. We let the
local environmental infectious pressure $\varphi_i = 0$ at the
beginning of the simulation and let 10\% of the herds start with 5\%
...
plot_nodes <- function(x) {
    ...
    for(i in seq(1, dim(wnp)[2], length.out = 18)) {
        Si <- which(!wnp[, i])
        Ii <- which(wnp[, i])
        plot(nodes$x[Si], nodes$y[Si], col = "yellow", pch = 20,
...
\begin{figure}
  ...
  \begin{center}
    \begin{subfigure}[b]{1.0\textwidth}
      \begin{center}
<<plot-SISe3sp-I, echo=FALSE, fig=TRUE>>=
...
@
    ...
    \begin{subfigure}[b]{1.0\textwidth}
      \begin{center}
<<plot-SISe3sp-II, echo=FALSE, fig=TRUE>>=
...
@
    ...
    \begin{subfigure}[b]{1.0\textwidth}
      \begin{center}
<<plot-SISe3sp-III, echo=FALSE, fig=TRUE>>=
...
@
        \caption{Between-node disease-spread by local spread among
          proximal nodes.}
        \label{fig:SISe3sp-local-spred}
      \end{center}
    \end{subfigure}
    \caption{The distribution of infected nodes at evenly spaced time
      points from one realisation of the \code{SISe3\_sp} model over
...
infected animals to the susceptible state during an \textit{external
  transfer} event so that infection is not tranfered, even though an
...
Figure~\ref{fig:SISe3sp-local-spred} shows the result from running one
trajectory with the modified model.
