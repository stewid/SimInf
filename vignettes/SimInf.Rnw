...
<<eval=FALSE>>=
N1 <- colSums(trajectory(result, compartments = c("S_1", "I_1"),
as.is = TRUE))
N2 <- colSums(trajectory(result, compartments = c("S_2", "I_2"),
as.is = TRUE))
N3 <- colSums(trajectory(result, compartments = c("S_3", "I_3"),
as.is = TRUE))
