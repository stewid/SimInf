...
analysis on the Comprehensive \proglang{R} Archive Network (CRAN).
For example, the package \pkg{amei} \citep{amei}, designed for finding
optimal intervention strategies to minimize total expected cost to
control a disease outbreak.  Another package is \pkg{surveillance}, a
framework for monitoring, modeling, and regression analysis of
infectious diseases, \citep{surveillance}.  \pkg{EpiModel}
\citep{EpiModel} is an \proglang{R} package that, includes a framework
for modeling spread of diseases on networks.  In \pkg{EpiModel} the
individual is the unit and transmission between individuals is
modelled through a contact network in discrete time.
...
called a vertex).  Moreover, each movement forms an \textit{edge}
...
\textit{external transfer} event moves individuals from compartments
in one node to compartments in a destination node. Finally, the
\textit{exit} event removes individuals from a node, for example, due
to death. The event types are classified into those that operate on
the compartments of a single node $E_1 = \{\text{\textit{enter},
  \textit{internal transfer}, \textit{exit}}\}$ and those that operate
on the compartments of two nodes $E_2 = \{\text{\textit{external
...
following order: \textit{exit}, \textit{enter}, \textit{internal
  transfer} and \textit{external transfer}.
...
post time step function.  The other solver implements an \textit{``all
...
\begin{array}{rcl}
  ...
  \end{array}
\end{align}
where the state transition per unit of time from susceptible to
infected depends on the concentration of the environmental
contamination $\varphi_i(t)$ in node $i$ and the indirect transmission
rate $\upsilon$.  Moreover, the state transition from infected to
susceptible depends on the recovery rate $\gamma$ per individual and
unit of time.  Finally, the environmental infectious pressure is
evolved by
\begin{equation}
...
the initial cattle population and \code{events} contains four years of
scheduled events data (births, deaths and movements).  Moreover, let us
define proximal neighbors as neighbors within 2500m and use the
utility function \code{distance\_matrix()} to estimate the distance
between farms within that cutoff.

<<distance-matrix>>=
d_ik <- distance_matrix(x = nodes$x, y = nodes$y, cutoff = 2500)
@

Here, we assume that 10\% of the farms have 5\% infected cattle at the
...
The \code{SISe\_sp} model contains parameters on a global and local
scale. Here, the parameter values were chosen such that the proportion
of infected nodes in a trajectory is about 10\% and display a seasonal
pattern.  The global parameters are: the spatial \code{coupling = 0.2}
($D$ in Equation~\ref{eq:envInfPressure-local-spread}), the shedding
rate \code{alpha = 1}, the recovery rate \code{gamma = 0.1} and the
...
season is local and is specified as the day of the year the season
ends.  Here, for simplicity, we let \code{end\_t1 = 91}, \code{end\_t2
  = 182}, \code{end\_t3 = 273} and \code{end\_t4 = 365} be identical
and recycled in all nodes.  Furthermore, the distances between nodes
are local data and are extracted from \code{distance = d\_ik}.
Finally, we let \code{phi = 0} at the beginning of the simulation
(becomes \code{v0} in the model object).  Let us now construct an
\code{SISe\_sp} object to simulate over four years.
...
replicate(5, {
    result <- run(model = model, threads = 1)
    lines(prevalence(model = result, formula = I ~ S + I, type = "nop"))
})
@

Assume there exists some sort of treatment by which the
\code{coupling} can be reduced by 50\%. Is that sufficient for
controlling the disease?  We use the utility function \code{gdata()}
to change the \code{coupling} parameter and then run some more
trajectories.
...
<<SISe_sp-prevalence, echo=FALSE, results=hide>>=
pdf("SimInf-SISe_sp-prevalence.pdf", width = 10, height = 5)
plot(NULL, xlim = c(0, 1500), ylim = c(0, 0.18),
ylab = "Prevalance", xlab = "Time")
set.seed(123)
replicate(5, {
  r <- run(model, threads = 1)
  lines(prevalence(r, I ~ S + I, "nop"))
})
gdata(model, "coupling") <- 0.1
replicate(5, {
  r <- run(model, threads = 1)
  lines(prevalence(r, I ~ S + I, "nop"), col = "blue", lty = 2)
})
dev.off()
@
...
\begin{figure}
  \begin{center}
  \includegraphics{SimInf-SISe_sp-prevalence.pdf}
  \end{center}
...
<<SIR-mparse-II>>=
n <- 1000
u0 = cbind(S = rep(99, n), I = rep(5, n), R = rep(0, n))
