...
during simulation.  Let us start with five empty nodes and then create
some \textit{enter} events to add susceptible individuals to the nodes
...
movements.  Also stochastic, because of the random selection process
...
\begin{figure}
  ...
  \caption{An \code{SIR} model in five nodes with scheduled events.
    ...
    was introduced at $t = 25$.  Further, some between-node movements
    occurred between $t = 35$ and $t = 45$.  Finally, at $t = 70$ and
    $t = 110$, twenty percent of the individuals were removed.  Right:
    For comparison, the deterministic dynamics when not introducing an
    infected individual.}
