...
framework.  Section \S\ref{sec:framework} gives a technical
description of the simulation framework.  In \S\ref{sec:examples} we
illustrate the use of the package by some worked examples.  Finally,
in \S\ref{sec:extend} we demonstrate how to extend \pkg{SimInf} with
user-defined models.
...
consider age structure \citep{Turner2006}, within and between-node
transmission via the environment \citep{Ayscue2009, Zhang2010},
ambient temperature \citep{Gautam2011}, herd size and between-herd
