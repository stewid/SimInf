...
The \pkg{SimInf} package is available on CRAN
...
A binary form of \pkg{SimInf} for macOS or Windows can be installed
directly from CRAN.  However, if you install \pkg{SimInf} from source
(from CRAN or a \code{.tar.gz} file), the installation process
requires a \proglang{C} compiler, and that the GNU Scientific Library
(GSL) \citep{Gallassi2009} is installed on your system and is in the
path.  Note that GSL (\url{https://www.gnu.org/software/gsl/}) is not
an \proglang{R} add-on package, but needs to be installed separately,
for example, from a terminal using: \code{'sudo apt-get install
...
computations in parallel.  To read more about installing \proglang{R}
add-on packages in general, the \texttt{'R Installation and
