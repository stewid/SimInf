...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.14\textwidth}
      ...
      \end{center}
    \end{subfigure}
  \caption{Illustration of livestock movements in a temporal network.
    (a) Each snapshot illustrates movements during one time unit, for
    example, during one day.  The network has $N = 4$ nodes where
    ($1$) is an infected node and ($2$--$4$) are non-infected nodes.
    Arrows indicate animal movements from a source node to a
    destination node, where labels denote the size of the shipment.
    In this example, infection may spread through livestock movements
    from $1 \rightarrow 3$ at $t=2$ (indicated by blue lines in $3$ at
    $t=3$) and then from $3 \rightarrow 2$ at $t=3$.  (b) The
    livestock movements represented as events $(\mathit{source},
...
functions, for example, \code{trajectory}, \code{prevalence} and
\code{plot}.
...
susceptible compartment.  Then, one infected individual is introduced
into a randomly sampled node from the population.  Note that we use
the 'L' suffix to create an integer value rather than a numeric value.
Run the model and accumulate \code{Icum}, for efficiency, use
\code{as.is = TRUE}, the internal matrix format, to extract
...
model_no_vac <- init(m, u0 = u0, tspan = 1:300, events = events,
E = E, N = N)
...
model_vac <- init(m, u0 = u0, tspan = 1:300, events =
rbind(events, vaccination), E = E, N = N)
