...
number of new cases per day from \code{n = 1000} realizations of a
\code{model} object in a \code{for} loop.  To clear infection that was
introduced in the previous trajectory, animals are first moved to the
susceptible compartment.  Note that we use the 'L' suffix to create an
integer value rather than a numeric value.  Then, one infected
individual is introduced into a randomly sampled node from the
population.  Run the model and accumulate \code{Icum}, for efficiency,
use \code{as.is = TRUE}, the internal matrix format, to extract
\code{Icum} in every node at each time-point in \code{tspan}.
