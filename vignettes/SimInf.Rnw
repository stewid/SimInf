...
three compartments.  If more than one compartment is involved in an
event, the individuals affected by the event will be sampled without
replacment from the specified compartments.  The numerical solver will
...
Now, let us illustrate in a small example, consisting only of five
nodes, how scheduled events can alter the composition of individual
nodes during simulation.  Let us start with five empty nodes and then
create some \textit{enter} events to add susceptible individuals to
the nodes during the first ten time-steps.
...
We then introduce one infected individual to the 5$^{th}$ node at $t =
25$.
...
Additionally, we create interactions among the nodes with some
movements during $t = 35$ to $t = 45$.  Each shipment contains $n = 5$
individuals.
...
Finally, we remove 20\% of the individuals from each node at $t = 70$
and $t = 110$.
...
Figure~\ref{fig:SIR-events} shows one realization after combining the
events.
...
\begin{figure}
  \begin{center}
<<SIR-events, echo=FALSE, fig=TRUE>>=
events = rbind(add, infect, move, remove)
u0 <- data.frame(S = rep(0, 5), I = rep(0, 5), R = rep(0, 5))
model <- SIR(u0 = u0, tspan = 1:180, events = events, beta = 0.16,
gamma = 0.077)
...
@
  \end{center}
  \caption{One realization of a stochastic \code{SIR} model
    incorporating scheduled events.  One infected individual was
    introduced at $t = 25$.}
  \label{fig:SIR-events}
\end{figure}

We can use \code{replicate} (or similar) to generate many realizations
from the model, together with some custom analysis of each trajectory.
Here, we find that infection spread from the 5$^{th}$ node in about
half of the trajectories.

<<SIR-replicate>>=
set.seed(123)
mean(replicate(1000, sum(trajectory(run(model, threads = 1),
node = 1:4)$I) > 0))
@
