...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.56\linewidth]{img/external.pdf}
  \end{center}
  ...
  \begin{center}
    \includegraphics[width=0.56\linewidth]{img/external-shift.pdf}
  \end{center}
  \caption{Illustration of a scheduled \textit{external transfer}
    event (event = 3) in the \code{SISe3} model at time = 4.  The
    ageing of three individuals in the second age category $\{S_2,
    I_2\}$ that are subsequenly moved.  Interpreting the figure from
...
model, which is also built into the \pkg{SimInf} package.  Two
additional features, compared to the \code{SIR} model, are
demonstrated in this example: ageing of individuals by
\textit{internal transfer} events, and usage of the real valued
continuous state $V$.  The \code{SISe3\_sp} model contains the two
compartments susceptible (\code{S}) and infected (\code{I}) divided
into the three age categories $j = \{1, 2, 3\}$ and the local
environmental compartment (\code{e}) contaminated with free living
pathogens (Figure~\ref{fig:SISe3_sp}).  Moreover, nodes are spatially
connected with local spread of the contaminated environmental
compartment among proximal nodes.  The transmission route of infection
to susceptible individuals is indirect via the local environment,
contaminated by infected individuals.  The number of individuals in
each of the six compartments $\{S_1, I_1, S_2, I_2, S_3, I_3\}$ in
node $i$ at time $t$ is \mbox{\code{u[, i]}}.  The \code{SISe3\_sp}
model has two state transitions within each of the three age
categories,
\begin{align}
\label{eq:vtectrans}
\begin{array}{rcl}
...
\code{SISe3\_sp} model have been specified to handle births, imports,
...
population data included with the package.  The data defines the
initial population divided into three age categories (0 days $\le$
\textit{age 1} $<$ 120 days, 120 days $\le$ \textit{age 2} $<$ 365
days and \textit{age 3} $\ge$ 365 days) in 1600 nodes and scheduled
events over $4 \times 365$ days.  The scheduled events i.e.\ the
data-driven part of the model are loaded with
<<load-SISe3sp-events>>=
...
proximal nodes, we have to generate a distance matrix.  Let us
consider a synthetic dataset (\code{nodes}) included in the
\pkg{SimInf} package, with a cattle population consisting of 1600
herds located in a 50 square kilometer region, and define proximal
neighbors as neighbors within 2500m.
<<load-nodes>>=
...
@
We are now ready to define the model and build an \code{SISe3_sp}
object with a call to the \code{SISe3_sp} generating method.  The
parameters for this example have been chosen such that the
between-node prevalence for a sample trajectory have a seasonal
pattern around 10\%.  We assume that the average duration of infection
is 10 days in all age categories, such that the recovery rate
$\gamma_{1,2,3} = 0.1$ per animal per day, and assume the indirect
transmission rate of the environmental infectious pressure, $\gamma$,
is equal in all age categories.  Moreover, we fix the average shedding
rate $\alpha = 1$ and allow the decay of the bacteria to vary in four
seasonal intervals, $\beta_1, ..., \beta_4$ over the year. The end
point (day of the year) for each season \texttt{beta\_t1}, ...,
\texttt{beta\_t4} is determined by \texttt{end\_t1}, ...,
\texttt{end\_t4}, respectively. We let the local environmental
infectious pressure $\varphi_i = 0$ at the beginning of the simulation
and let 10\% of herds start with 5\% infected individuals.
<<create-SISe3sp-u0>>=
set.seed(123)
i <- sample(1:1600, 160)
...
The model can be summarized with the \code{summary} method, which
displays the model name, the number of nodes $N_n$, the number of
compartments $N_c$, the number of transitions $N_t$, and the total
number of scheduled events and information regarding each event type.
If the model has not yet been run, the size of \code{U} and \code{V}
is \code{0 x 0}.
...
\begin{figure}[!htb]
  \begin{center}
<<display-SISe3sp-model, fig=TRUE, height=4>>=
plot(events(model))
@
  \end{center}
...
\begin{figure}
  \begin{center}
<<display-population-dynamics, fig=TRUE>>=
library("lattice")
result <- run(model, threads = 1, seed = 123)
wnp <- prevalence(result, type = "wnp")
i <- which(rowSums(wnp) > 0)[1:12]
wnp <- data.frame(Node = factor(paste0("Node: ", i), levels = paste0(
"Node: ", i)), Days = rep(result@tspan, each = length(i)),Prevalence =
as.numeric(wnp[i, ]))
xyplot(Prevalence ~ Days | Node, data = wnp, type = "l", col =
"black", layout = c(6, 2))
@
  \end{center}
  \caption{The within-node prevalence in twelve nodes after running
    one trajectory over $4 \times 365$ days of the \code{SISe3\_sp}
    model using the scheduled events data contained in the
    \pkg{SimInf} package.}
  \label{fig:within-node-prevalence}
\end{figure}
...
@
  \end{center}
  \caption{One realisation of an epidemic curve displaying the number
...
While \pkg{SimInf} was designed to study disease spread over temporal
networks, it is not limited to that use case but can also be used to
study the dynamics of other systems.  In a final \code{mparse} example
we will illustrate the Rosenzweig-MacArthur predator-prey model
demonstrated in the \pkg{GillespieSSA} package \citep{Rosenzweig1963,
  Pineda-Krch2008}.  The model has a density-dependent growth in the
prey and and a nonlinear Type-2 functional response in the predator
...
@
  \end{center}
  \caption{One realisation of the \code{mparse} Rosenzweig-MacArthur
    predator-prey model.  In this trajectory, the predator (\code{F})
    go extinct after about 50 days while the prey (\code{R}) then
    fluctuates around a plateu of 1000
...
steps to achieve this.  To read more about creating \proglang{R}
add-on packages in general, the \texttt{'Writing R extensions'}
(\url{https://cran.r-project.org/doc/manuals/r-release/R-exts.html})
manual is the official guide and describes in detail the process.
Another useful resource is the \texttt{'R packages'} book by
\cite{Hadley2015} (\url{http://r-pkgs.had.co.nz/}).

Consider we wish to create a new add-on package \pkg{PredatorPrey}
based on the Rosenzweig-MacArthur predator-prey model from the
previous section.  To facilitate this, the \pkg{SimInf} package
includes a method (\code{package_skeleton}) that automates some of the
setup for a new source package.  It creates directories, saves
...
neccessary definitions and declarations for these functions and should
be included.  The \code{SimInf_model_run} function is the interface
from \proglang{R} to the core solver in \proglang{C} and list all
function pointers to the transition rate functions in a vector in the
order the state transitions appear in the dependency graph \code{G},
see Listing~\ref{lst:SIRrun} in the appendix for an example from the
...
This paper introduces the R package \pkg{SimInf}, which supports
data-driven simulations of disease spread over spatio-temporal
networks.  The package offers a very efficient and highly flexible
tool to incorporate real data in simulations at realistic scales.
