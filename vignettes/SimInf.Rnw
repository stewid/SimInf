...
\Abstract{
  ...
  as births, slaughter and individual movements as scheduled events at
  predefined time-points. Using \proglang{C} code for the numerical
  solvers and \proglang{OpenMP} to divide work over multiple
  processors ensures high performance when simulating a sample
  outcome. One of our design goal was to make \pkg{SimInf} extendable
  and enable usage of the numerical solvers from other \proglang{R}
  extension packages in order to facilitate complex epidemiological
  research. In this paper, we provide a technical description of the
  framework and demonstrate its use on some basic examples. We also
  discuss how to specify and extend the framework with user-defined
  models.
...
Many diseases have a zoonotic origin, i.e., are naturally transmitted
from animals to humans, for instance through direct contact with
infected animals or by consumption of contaminated food or water.  In
the European Union (EU), over 350,000 human cases of zoonoses are
reported annually \citep{EUzoonoses}.  Cattle can act as a reservoir
for \textit{Salmonella} and Verotoxin-producing \textit{Escherichia
  coli} (VTEC), two important examples of zoonotic food-borne
pathogens.  Therefore, a better understanding of how such diseases
spread in the cattle population is necessary in order to develop
effective strategies for control \citep{Newell2010}.  European Union
legislation requires member states to keep a registry of all bovine
animals in national databases \citep{EC-No-1760-2000, EC-No-911-2004}
with information on births, movements and slaugther.  Consequently,
detailed spatiotemporal demographic data and the transportation
network are available and can be used to quantify the effect of, for
example, seasonal forcing \citep{Kao2006} and population age-structure
\citep{Brooks-Pollock2013} on the infection dynamics.  Since cattle
are aggregated into spatially segregated farms, it is natural to use a
metapopulation framework and partition the cattle population into
interacting subpopulations \citep{Fulford2002, Keeling2010}.  To
better understand spread and control at regional and national level,
available data could be used for epidemiologically relevant factors
when simulating the infection process within each subpopulation,
coupled with spread among subpopulations governed by spatial proximity
and livestock movements.  However, incorporating large amounts of data
in simulations is computationally challenging and requires efficient
algorithms.
...
demographics and network data.  This allows the user to study
transmission in realistic large scale disease spread models.  The
framework integrates infection dynamics in each subpopulation as
continuous-time Markov chains (CTMC) using the Gillespie stochastic
simulation algorithm (SSA) \citep{Gillespie1977} and incorporates
available data such as livestock movements, births, slaughter or aging
as scheduled events.  A scheduled event is used to modify the state of
a subpopulation at a predefined time-point.  Using compiled
\proglang{C} \citep{Kernighan1988} code, rather than interpreted
\proglang{R} code, for the numerical solvers ensures high performance
when simulating a model.  To further improve performance,
...
population, incorporating over eight years of scheduled events data in
a network of 37221 premises \citep{Bauer2016, Widgren2016,
  Widgren2018}.  Even if development of \pkg{SimInf} was inspired by
zoonotic diseases and models driven by livestock data, the design is
of completely general character and applies to arbitrary
metapopulation models.  One of our design goal was to make
...
Various packages in the CRAN repository implement SSA to simulate a
continuous-time stochastic process.  The \pkg{GillespieSSA} package
\citep{Pineda-Krch2008}, on CRAN since 2007, but apparently not
actively maintained after 2012, implements both the direct method and
three approximate methods.  The \pkg{hybridModels} package
...
uses \proglang{C++} to implement the direct method and adaptive tau
leaping \citep{Cao2007}.
...
Moreover, \pkg{SimInf} simulates the infection process in
subpopulations that are coupled to form a larger system, whereas
\pkg{EpiModel} uses the individual as the epidemiological unit and
simulates spread between individuals through a contact network.

The remainder of this paper is organized as follows.  In
\S\ref{sec:modeling} we summarize the mathematical foundation for our
framework. Section \S\ref{sec:framework} gives a technical description
of the simulation framework. In \S\ref{sec:examples} we illustrate the
use of the package by some worked examples. In \S\ref{sec:extend} we
demonstrate how to extend \pkg{SimInf} with user-defined models.
Finally, in \S\ref{sec:benchmark} we provide a small benchmark between
...
\begin{figure}
  ...
  \end{center}
  \caption{Illustration of movements as a temporal network.
    Each time step depicts movements during one time unit, for
    example, a day.  The network has $N = 4$ nodes where node $1$ is
    infected and nodes $2$--$4$ are non-infected.  Arrows indicate
    movements of individuals from a source node to a destination node and
    labels denote the size of the shipment.  Here, infection may
    spread from node $1$ to node $3$ at $t=2$ and then from node $3$
    to node $2$ at $t=3$.
...
network terminology, each farm is represented by a \textit{node} (also
called a vertice).  Moreover, each movement forms an \textit{edge}
(also called a link) between two nodes and following all edges through
time, an infection may ``flow'' in the network and spread from node to
node. Let $\Nnodes$ denote the number of nodes in a population, and
let $i, j, k \in \{1, ..., \Nnodes\}$ denote three distinct nodes.  As
...
In \S\ref{subsec:local}--\ref{subsec:numerics}, we provide an overview
of the epidemiological modeling framework employed in \pkg{SimInf}.
The overall approach consists of CTMCs as a general model of the
dynamics of the epidemiological state. Importantly, we also allow for
variables obeying ordinary differential equations (ODEs). For example,
this readily supports modeling infections that have an indirect
transmission route, e.g., via shedding of a pathogen to the
environment \citep{Ayscue2009, Breban2009}.  Additionally, the
...
\S\ref{subsec:numerics}. We draw much of the material here from
...
graph $\mathcal{G}$ as the matrix
$\Connect \in \Intdom_{+}^{\Ncompartments \times \Nnodes}$.
...
the Euler forward method in time with time-step
$\Delta t_{n} = t_{n+1}-t_{n}$ for the concentration variable $\Y$.
...
\begin{figure}
  ...
  \caption{Schematic overview of the functionality in
    \pkg{SimInf}. The central object is the S4 class
    \code{Simlnf\_model} which contains the specification and data for
    ...
    solvers from other \proglang{R} packages via the \emph{LinkingTo}
    feature in \proglang{R}. \label{fig:overview}}
\end{figure}
...
oriented programming and the \code{S4} class \citep{Chambers2008}
...
\S\ref{sec:examples}.  A model can also be created from a model
specification using the \code{mparse} method, further described in
\S\ref{sec:extend}.  After a model is created, a simulation is started
with a call to the \code{run} method and if execution is successful,
it returns a modified \code{SimInf\_model} object with a single
stochastic solution trajectory attached to it.  \pkg{SimInf} provides
several utility functions to inspect simulated data, for example,
\code{show}, \code{summary} and \code{plot}.  To facilitate custom
analysis, \pkg{SimInf} provides the \code{trajectory} and
...
in an infected node, but sometimes the infectious disease goes extinct
before it causes an epidemic, as shown in Figure~\ref{fig:SIR-model}.
...
\begin{figure}
  ...
  \caption{Output from a stochastic \code{SIR} model in 1000 nodes
    ...
    from a subset of 10 nodes.  \label{fig:SIR-model}}
\end{figure}
...
\textit{enter} events to add susceptible individuals to each node
during the first ten time-steps.
...
We then create one \textit{enter} event to introduce an infected
...
Additionally, we create some \textit{external transfer} events to form
interactions among the nodes with movements during $t = 35$ to $t =
...
Finally, we create \textit{exit} events to remove 20\% of the
individuals from each node at $t = 70$ and $t = 110$.
...
Here we will present an example to illustrate how local data and
\textit{internal transfer} events can be used to formulate more
complex models.  Moreover, we will introduce the \code{prevalence()}
method, another important function for post-processing trajectories.
For this, we will use the predefined \code{SISe3\_sp} model that was
developed to study spread of VTEC in cattle \citep{Widgren2018}.
Briefly, a VTEC infection in cattle can be formulated as a
susceptible-infected-susceptible (SIS) model.  However, previous
modeling of VTEC in a cattle has shown that it is important to
consider age structure \citep{Turner2006}, within and between-farm
transmission via the environment \citep{Ayscue2009, Zhang2010},
ambient temperature \citep{Gautam2011}, herd size and between-farm
spread from livestock movements \citep{Zhang2010}.  Therefore, the SIS
model was divided into three age categories $j = \{1, 2, 3\}$.
Additionally, an environmental compartment (\code{e}) was added to
model the contamination of VTEC in each node from infected cattle
shedding the pathogen into the environment.  Moreover, to have
between-node spread unrelated to moving infected animals, a spatial
coupling of the environmental contamination among proximal nodes was
included (\code{sp}).  Consequently, the model has two state
...
Moreover, the state transition from infected to susceptible depends on
the recovery rate $\gamma_j$.  Finally, the environmental infectious
...
\end{equation}
where the $\alpha$ is the average shedding rate of the pathogen to the
...
is captured by $\beta$.  When running a trajectory of the
\code{SISe3\_sp} model, the environmental infectious pressure
$\varphi_i(t)$ in each node is evolved in the post-time-step function
by the Euler forward method (see the file 'src/models/SISe3\_sp.c' for
...
\mathbf{N} =
\bordermatrix{
      & \enspace 1 & \enspace 2 & \enspace 3 \cr
  S_1 & \hphantom{-}2 & \hphantom{-}0 &  \hphantom{-}0  \cr
  I_1 & \hphantom{-}2 & \hphantom{-}0 & -1 \cr
  S_2 & \hphantom{-}0 & \hphantom{-}2 &  \hphantom{-}0 \cr
  I_2 & \hphantom{-}0 & \hphantom{-}2 & -1 \cr
  S_3 & \hphantom{-}0 & \hphantom{-}0 &  \hphantom{-}0 \cr
  I_3 & \hphantom{-}0 & \hphantom{-}0 & -1 }
\]
}
...
Let us now demonstrate how to construct and use the \code{SISe3\_sp}
model using a synthetic dataset of 1600 farms located in a 50 square
kilometer region.  Load the \code{data.frame} data with the following
commands
...
where the location of each farm is in \code{nodes} , \code{u0\_SISe3}
defines the initial cattle population divided into three age
categories (0 days $\le S_1,I_1 <$ 120 days, 120 days $\le S_2,I_2 <$
365 days and $S_3, I_3 \ge$ 365 days) and finally,
\code{events\_SISe3} contains four years of scheduled events data.
Let proximal neighbors in
Equation~\ref{eq:envInfPressure-local-spread} be defined as neighbors
within 2500m and use the utility function \code{distance\_matrix()} to
estimate the distance between nodes within that cutoff.

<<load-nodes>>=
d_ik <- distance_matrix(x = nodes$x, y = nodes$y, cutoff = 2500)
@

Assume 10\% of the farms have 5\% infected cattle at the beginning of
the simulation.
...
to specify which compartments to operate on when processing a
scheduled event.  The first column is for \textit{enter} events
(births); add newborn animals to the susceptible compartment $S$.  The
second column is for \textit{exit} events (deaths) and
\textit{external transfer} events (livestock movements); sample
animals from the $S$, $I$, $R$ or $V$ compartments.  Finally, the
third column is for \textit{internal transfer} events (vaccination);
sample individuals from the $S$, $I$ or $R$ compartments.
Furthermore, we define the shift matrix $N$ to process
...
Although \pkg{SimInf} was designed to study disease spread using
demographic data, it is not limited to that use case but can also be
...
declarations for these functions and must be included.  The
...
\begin{figure}
  ...
  \caption{Left: Phase plane trajectories from 1000 realizations of
    ...
    around a plateu of 1000 individuals.  \label{fig:predator-prey}}
\end{figure}
