...
numerical solvers ensures high performance when simulating a model.
To further improve performance, \proglang{OpenMP} \citep{OpenMP2008}
is used to divide work over multiple processors and perform
computations in parallel.  Furthermore, the framework has a
well-defined interface to incorporate data that is shared among all
subpopulations (global) and data that is specific to each
subpopulation (local), allowing sophisticated models to be
straightforwardly formulated.  The proposed approach was used to study
spread and control of VTEC in the complete Swedish cattle population,
incorporating over eight years of scheduled events data in a network
of 37221 premises \citep{Bauer2016, Widgren2016, Widgren2018}.  Even
if development of \pkg{SimInf} was inspired by zoonotic diseases and
models driven by livestock data, the design is of completely general
character and applies to arbitrary metapopulation models.  One of our
...
this, \pkg{SimInf} has functionality to generate the required
\proglang{C} and \proglang{R} code from a model specification.
...
\begin{figure}
  ...
  \caption{Schematic overview of the functionality in
    ...
    pre-defined template, for example, \code{SIR} or \code{SEIR}.  A
...
Figure~\ref{fig:overview} shows a schematic overview of the
functionality in \pkg{SimInf}.
...
framework.  The \code{SimInf\_model} class contains data structures
that define a compartment model and one slot \code{events} associated
to a \code{SimInf\_events} object that handle scheduled events.  The
predefined disease-spread models in \pkg{SimInf} have a generating
function \citep{Chambers2008}, with the same name as the model, to
initialize data structures for that specific model, see examples in
...
data.  We use \code{threads = 1} and set the seed for reproducibility.
