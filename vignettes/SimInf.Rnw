...
\pkg{SimInf} provides the \code{trajectory()} method to extract the
number of individuals in each compartment at the time points specified
in \code{tspan} as a \code{data.frame}.  Below is an excerpt of the
trajectory data in the first node.  To extract all data from all
nodes, you only have to remove the node argument.  Consult the help
page for other \code{trajectory} parameter options.
