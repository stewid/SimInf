...
\begin{figure}
  ...
  \caption{Schematic overview of the functionality in
    ...
    a model.  To create a new model object, use \code{mparse} or a
    pre-defined template, for example, \code{SIR} or \code{SEIR}.  A
    stochastic trajectory is simulated from a model by calling
    \code{run}.  For computational efficiency, the numerical solvers
    are implemented in \proglang{C} code.  There are several functions
    to facilitate post-processing and analysing simulated data, for
    example, \code{trajectory}, \code{prevalence} and \code{plot}.
    \pkg{SimInf} was designed to support usage of the numerical
