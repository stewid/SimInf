...
replacement from the specified compartments.  The numerical solver will
perform an extensive error checking of the event before it is
...
complex models.  Furthermore, we will also introduce the
\code{prevalence()} method, another important function to post-process
trajectories.  For this, we will use the predefined \code{SISe3\_sp}
model that was developed to study spread of VTEC in cattle
\citep{Widgren2018}.  Briefly, a VTEC infection in cattle can be
formulated as a susceptible-infected-susceptible (SIS) model.
However, previous modeling of VTEC in a cattle has shown that it is
important to consider age structure \citep{Turner2006}, within and
between-node transmission via the environment \citep{Ayscue2009,
  Zhang2010}, ambient temperature \citep{Gautam2011}, herd size and
between-herd spread from livestock movements \citep{Zhang2010}.
Therefore, the SIS model was divided into three age categories $j =
\{1, 2, 3\}$.  Additionally, an environmental compartment (\code{e})
was added to model the contamination of VTEC in each node from
infected cattle shedding the pathogen into the environment.  Moreover,
to have between-node spread unrelated to moving infected animals, a
spatial coupling of the environmental contamination among proximal
nodes was included (\code{sp}).  Consequently, the model has two state
...
fourh node (use \code{node = 4}) to illustrate stochastic extinction of
