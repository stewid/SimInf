...
now contains the $\{S, I, I_{cum}, R, V \}$ compartments.  We assume,
for the sake of simplicity, that vaccinated individuals become immune
and non-infectious immediately.
...
Load example data of a population of 1600 nodes (cattle herds) and
scheduled events (births, deaths, and livestock movements) contained
in the package.  Let $I_{cum} = 0$ and $R = 0$.
