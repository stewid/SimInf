...
\begin{table}
  ...
  \caption{Examples of the specification of a single row of scheduled
    event data in the \code{SISe3} model to add, move or remove
    individuals during the simulation.}
  \label{table:SISe3:events}
\end{table}
...
\begin{figure}
  ...
  \caption{Illustration of a scheduled \textit{exit} event (event = 0)
    in the \code{SISe3} model at time = 4.  The removal of one
...
\begin{figure}
  ...
  \caption{Illustration of a scheduled \textit{enter} event (event =
    1) in the \code{SISe3} model at time = 4.  Add three susceptible
...
\begin{figure}
  ...
  \caption{Illustration of a scheduled \textit{internal transfer}
    event (event = 2) in the \code{SISe3} model at time = 4.  The
