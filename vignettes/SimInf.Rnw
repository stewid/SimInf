...
each node during the first ten time-steps.
...
We then introduce one infected individual to node five at $t = 25$.
...
Additionally, we create interactions among nodes with some movements
during $t = 35$ to $t = 45$.  Each shipment contains $n = 5$
individuals.
