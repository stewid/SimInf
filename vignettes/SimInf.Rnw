...
Algorithm~\ref{alg:core} in Appendix~\ref{sec:pseudo-code} with
pseudo-code for the core simulation solver.  The scheduled events are
...
function(model) {prevalence(run(model), cases = "I", pop = c("S", "I", "R"), type = "pop", as.is = TRUE)[75]})
nop <- run_outer(x, y, model, gamma ~ beta,
function(model) {prevalence(run(model), cases = "I", pop = c("S", "I", "R"), type = "nop", as.is = TRUE)[75]})
@
...
plot_nodes <- function(x) {
  ...
  wnp <- prevalence(x, cases, pop, type = "wnp", as.is = TRUE) > 0
...
\section{Pseudo-code for the core simulation solve}
