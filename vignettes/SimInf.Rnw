...
compartments within one node, for example, aging of individuals in a
...
destination node.  Finally, the \textit{exit} event removes
individuals from a node, for example, due to slaughter.  The event
types are classified into those that operate on the compartments of a
single node $E_1 = \{\text{\textit{enter}, \textit{internal transfer},
...
inividuals from node 1 to node 3, there was no outbreak in node 3.
...
(births, imports), they are added to the susceptible state in the age
category specified by column 1, 2 or 3 in \code{E}.  The columns 4, 5
and 6 in \code{E} are used for the other events (ageing, moving, exit)
to sample individuals from the susceptible and infected states from
each of the three ages catgories.  To move the sampled individuals in
a \textit{internal transfer} event (ageing) from age category 1 to age
...
\code{N[, 1]}.  Similarly, \code{N[, 2]} move susceptible and infected
...
scheduled events over $4 \times 365$ days.  The two datasets for the
...
wich defines a cattle population consisting of 1600 herds located in a
...
season \texttt{beta\_t1}, ..., \texttt{beta\_t4} is determined by
...
The scheduled events give rise to a seasoanal variation of the
...
Age1 <- rep("0 <= Age < 120 days", length(N1))
Age2 <- rep("120 <= Age < 356 days", length(N2))
Age3 <- rep("365 <= Age days", length(N3))
Days <- rep(result@tspan, 3)
population <- data.frame(Individuals = c(N1, N2, N3), Days = Days,
Age = c(Age1, Age2, Age3))
xyplot(Individuals ~ Days | Age, data = population, type = "l",
scales = list(relation="free"), layout = c(1, 3))
@
...
@
  ...
  \caption{The population dynamics after running one trajectory over
    ...
    events data contained in the \pkg{SimInf} package.  Please note
    that the y-axis do not start at zero and have different scales.}
