...
Administration} (\url{https://cran.r-project.org/manuals.html}) manual
describes the process in detail.  After installing the package
...
\begin{table}
  ...
  \begin{tabularx}{\textwidth}{l X}
    ...
    \code{N} & Determines how individuals in enter, internal transfer
    and external transfer events are shifted to enter another
    ...
    compartment, for example, a value of \code{1} in an entry means
    that sampled individuals in this compartment are moved to the next
    compartment.  Which column to use for each event is specified by
    the \code{shift} vector (see below).  \code{N} is an integer
    matrix.\\
...
\cite{Hadley2015} (\url{https://r-pkgs.org/}).
...
\begin{scriptsize}
  ...
  SEXP SIR_run(SEXP model, SEXP options)
  {
