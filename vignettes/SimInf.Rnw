...
has been developed since the paper was published.  The results
reported here are based on \pkg{SimInf} version
\Sexpr{packageVersion("SimInf")} unless otherwise stated.
