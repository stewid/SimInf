...
\subsection[A first example: The SIR model without scheduled events]{%
  A first example: The \code{SIR} model without using scheduled
  events}
...
information about the model, such as global data parameters and the
extremes, the mean and the quartiles of the count in each compartment
across all nodes.
...
the simulation data has been generated. To facilitate this,
\pkg{SimInf} provides the \code{trajectory()} method to extract a
\code{data.frame} with the number of individuals in each compartment
at the time points specified in \code{tspan}.  Below is an excerpt of
the trajectory data from the first node that clearly shows there was
an outbreak there.  To extract all data from every node, you only have
to remove the node argument.  Consult the help page for other
\code{trajectory} parameter options.
