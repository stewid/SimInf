...
on macOS with the Homebrew package manager.  Furthermore, when you
...
functions, for example, \code{prevalence} and \code{plot}.
...
\begin{table}[!ht]
  ...
  \begin{tabularx}{\textwidth}{l X}
    ...
    \code{E} & Event matrix.  Each row corresponds to one compartment
    in the model.  The non-zero entries in a column indicates the
    compartments to include in an event.  Sparse matrix of object
    class \code{dgCMatrix}.\\

    \code{N} & Shift matrix.  Each row represents one compartment in
    the model and the values determine how to move sampled individuals
    in \textit{internal transfer} and \textit{external transfer}
    events.  Integer matrix.\\
...
function(model) {prevalence(run(model), I~., type = "pop", as.is = TRUE)[75]})
nop <- run_outer(x, y, model, gamma ~ beta,
function(model) {prevalence(run(model), I~., type = "nop", as.is = TRUE)[75]})
@
...
title("Node prevalence")
mtext(expression(beta), side = 1, line = 3)
mtext(expression(gamma), side = 2, line = 2)
par(opar)
@
  \end{center}
  \caption{Contour plot of the population- and node prevalence after
    simulating 75 days of an \code{SIR} model in 500 nodes, starting
    with 99 susceptible, 1 infected, and 0 recovered individuals in
    each node at the first day. \label{fig:SIR-run-outer}}
\end{figure}
...
events <- data.frame(event = c(1, 3, 0, 3, 3, 3), time = c(2, 3, 4, 4, 4, 5),
node = c(2, 1, 2, 1, 1, 1), dest = c(0, 3, 0, 3, 4, 5),
n = c(2, 1, 1, 1, 1, 2), proportion = c(0, 0, 0, 0, 0, 0),
select = c(1, 2, 2, 2, 2, 2), shift = c(0, 0, 0, 0, 0, 0))
@
...
trajectory(run(model, threads = 1, seed = 22))
@
...
events <- data.frame(event = c(3, 3, 3, 3), time = c(3, 25, 5, 10),
node = c(1, 1, 1, 1), dest = c(3, 2, 3, 3), n = c(1, 9, 1, 5),
proportion = c(0, 0, 0, 0), select = c(2, 2, 2, 2), shift = c(0, 0, 0, 0))
@
...
in the \code{U} matrix.  The \code{U} method take a
...
N1 <- colSums(trajectory(result, compartments = c("S_1", "I_1"), as.is = TRUE))
N2 <- colSums(trajectory(result, compartments = c("S_2", "I_2"), as.is = TRUE))
N3 <- colSums(trajectory(result, compartments = c("S_3", "I_3"), as.is = TRUE))
...
transitions in the form of \code{"X -> propensity -> Y"} and
generates: the \proglang{C} code for the model, the state-change
matrix \code{S}, and the dependency graph \code{G}.  The left hand
...
list the compartments in the order they should appear in the \code{U}
matrix.

\subsubsection[Introductory examples of using mparse]{Introductory examples of using \code{mparse}}
...
an object of class \code{'SimInf\_mparse'} via a call to
\code{mparse}.  Before we can simulate from the model, it needs to be
initialized with the initial condition \code{u0} and \code{tspan}.
...
happens.  We then calculate successive differences at each time-point
in \code{'Icum'} and plot the result as an epidemic curve
...
result <- run(model, threads = 1, seed = 22)
Icum <- trajectory(result)$Icum
cases <- stepfun(result@tspan[-1], diff(c(0, Icum)))
...
@
  ...
  \caption{One realization of an epidemic curve displaying the number
    ...
    the \code{mparse} \code{SIR} model (Equation~\ref{eq:SIR}),
    starting with 99 susceptible, 1 infected and 0 recovered
    individuals.  \label{fig:SIR-mparse-incidence}}
\end{figure}

\subsubsection[Incorporate scheduled events in an mparse model]{Incorporate scheduled events in an \code{mparse} model}
\label{sec:mparse-scheduled-events}

Let us expand on the previous example to illustrate how models
generated using \code{mparse} can incorporate scheduled events.
Consider an epidemic in a population consisting of 1600 nodes, for
example, cattle herds, that are connected to each other by livestock
movements.  Assume the outbreak is detected on day twenty-one after
introduction and that we wish to explore how vaccination could limit
the outbreak, if resources for vaccination can handle 50 herds per day
and 80\% of the animals in each herd.  Let us add one compartment $V$
to the model, to represent vaccinated individuals, so that the model
now contains the $\{S, I, I_{cum}, R, V \}$ compartments.  As before,
let \code{b} denote the transmission rate and \code{g} the recovery
rate.

<<mparse-scheduled-events>>=
transitions <- c("S -> b*S*I/(S+I+R+V) -> I", "I -> g*I -> R")
...
deaths, and livestock movements.  Let $I_{cum} = 0$ and $R = 0$.
...
events (\code{event = 2}).  Use \code{select = 3} and \code{shift = 1}
to move animals from the susceptible, infectious and recovered
compartments to the vaccinated compartment, see the definitions of $E$
and $N$ below.  Let us start the vaccinations in nodes 1--50 on day
twenty-one, and continue until all herds are vaccinated on day
fifty-two, and use \code{proportion = 0.8} to vaccinate 80\% of the
animals in each herd.  We assume, for the sake of simplicity, that
vaccinated individuals become immune and non-infectious immediately.

<<mparse-scheduled-events-vaccination>>=
vaccination <- data.frame(event = 2, time = rep(21:52, each = 50),
node = 1:1600, dest = 0, n = 0, proportion = 0.8, select = 3,
shift = 1)
@

To simulate from this model, we have to define the event matrix $E$
...
compartments.  Finally, the third column is for \textit{internal
  transfer} events; sample individuals from the $S$, $I$ or $R$
compartments for vaccination.
...
First move all infected individuals to the susceptible compartment to
clear the node where infection was randomly introduced with one
infected individual in the previous trajectory.  Then sample one node
in the population to seed infection with one infected individual.  Run
the model and accumulate \code{Icum}.  For effiency, use \code{as.is =
  TRUE}, the internal matrix format, to extract \code{Icum} for every
node at each time-point in tspan, and then summarise \code{Icum} at
each time-point.
...
epicurve <- function(model, n = 100) {
  ...
  for (i in seq_len(n)) {
    ...
    result <- run(model)
    Icum <- Icum + colSums(trajectory(result, "Icum", as.is = TRUE))
  }
...
Create a new model object with the events, $E$ and $N$ defined.

<<mparse-network-model>>=
tspan <- seq(from = 1, to = 4*365, by = 7)
model <- init(m, u0 = u0, tspan = tspan, events = events, E = E, N = N)
@

%% \begin{figure}
%%   \begin{center}
%% <<mparse-mparse-plot, echo=FALSE, fig=TRUE>>=
%% cases <- stepfun(model@tspan[-1], diff(c(0, Icum / N)))
%% plot(cases, main = "", xlim = c(0, 300), xlab = "Time",
%% ylab = "Number of cases", do.points = FALSE)
%% @
%%   \end{center}
%%   \caption{FIXME.  \label{fig:mparse-network}}
%% \end{figure}

\clearpage

\subsubsection{Predator-prey model}
\label{sec:mparse-predator-prey}
...
model consists of five transitions (Equation~\ref{eq:predator-prey}):
...
\subsection[Use the SimInf framework from another package]{Use the \pkg{SimInf} framework from another package}
...
updated to meet the specific requirements of the model, for example,
if the model should handle scheduled events, then \code{E} must be
specified to select the subset of compartments involved in each
specific event.  Furthermore, if one of the events is an
\textit{internal transfer} event, \code{N} must be specified to shift
individuals between compartments within one holding.  It is also
possible to define parameters that are either local (\code{ldata}) to
each node or shared (\code{gdata}) between all nodes.
...
Listing~\ref{lst:SIRrun} in Appendix~\ref{sec:C-code-SIR} for an
example from the \code{SIR} model and the use of the address of
...
\code{SimInd_model_run} function plus the vector of function pointers
to the transition rate functions and the function pointer to the post
time step function.  To facilitate extraction of specific values from
the data structures passed as arguments to the transition rate
functions and the post time step function, we recommend using
enumeration declarations to name the values, see
Listing~\ref{lst:trSIR} in Appendix~\ref{sec:C-code-SIR} for an
example.
