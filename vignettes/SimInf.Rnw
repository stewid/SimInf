...
to \code{select} in \code{E}, where the non-zero entries in the
specified column define the involved compartments.  For the predefined
\code{SIR} model, \code{E} is defined as
