...
extracted using the \code{trajectory} method.  The row names indicate
the compartments in the model and the colnames are the time points in
\code{tspan}.  Please note that the \code{U} matrix is truncated here
for readability.

<<SIR-U>>=
trajectory(result)[1:10 ,]
@
