...
\code{print} method of the \code{SimInf_model} class shows some basic
information about the model, such as the name and value of global data
parameters shared among nodes and the extremes, the mean and the
quartiles of the count in each compartment across all nodes.
...
sixth node (use \code{i = 4}) to illustrate stochastic extinction of
...
plot(R ~ time, trajectory(model = result, i = 4), type = "l",
xlab = "Time", ylab = "N")
lines(F ~ time, trajectory(model = result, i = 4), type = "l", lty = 2)
