...
model <- init(model = m, u0 = cbind(S = 99, I = 1, R = 0), tspan = 1:180)
@
...
model <- init(model = m, cbind(S = 99, I = 1, Icum = 0, R = 0), 1:180)
result <- run(model = model, threads = 1, seed = 22)
Icum <- trajectory(model = result)$Icum
...
epicurve <- function(model, n = 1000) {
  ...
  for (i in seq_len(n)) {
    ...
    traj <- trajectory(model = result, "Icum", as.is = TRUE)
