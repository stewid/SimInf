...
\begin{figure}
  ...
  \end{center}
  \caption{Illustration of three snapshots of movements in a temporal
    network.  Each snapshot illustrates movements during one time
    unit, for example, one day.  The network has $N = 5$ nodes
    ($a$--$e$) and arrows represent movements of individuals from a
    source node to a destination node (labels denote the size of each
    shipment).
